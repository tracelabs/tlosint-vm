...
# ============================================================
# OPSEC / Anonymity Add-ons (Ghost findings)
# ============================================================

install_opsec_tools() {
  log "Installing OPSEC/Anonymity tools (Tor, AnonSurf, MAC spoofing, metadata cleaner)"

  # Tor is expected to be preinstalled in the base VM image.
  # Only install torsocks here (lightweight per-command Tor wrapper).
  apt-get install -y torsocks || true

  # Warn (but don't fail) if Tor isn't present.
  if ! command -v tor >/dev/null 2>&1; then
    warn "Tor (tor) not found. Tor should be preinstalled in the base VM image; per-command routing will not work until Tor is installed."
  fi
  ...
  apt-get install -y mat2 || true

  # AnonSurf (not always available in Debian repos); best-effort install
  if ! apt-get install -y anonsurf 2>/dev/null; then
    warn "AnonSurf package not available via apt on this distro (best-effort)."
  fi

  # Convenience wrappers (used by menu installer)
  cat > /usr/local/bin/tl-anon-start <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
echo "[!] anonsurf not found. Falling back to Tor service + torsocks (if available)."
if ! command -v tor >/dev/null 2>&1; then
  echo "[WARN] Tor (tor) is not installed. Install Tor in the base VM image to use torsocks routing."
else
  sudo systemctl enable tor >/dev/null 2>&1 || true
  sudo systemctl start tor >/dev/null 2>&1 || true
  echo "Tor service started. For per-command routing, use: torsocks <command>"
fi
EOF
  chmod 0755 /usr/local/bin/tl-anon-start

  cat > /usr/local/bin/tl-anon-stop <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
echo "[!] anonsurf not found. Stopping Tor service only."
sudo systemctl stop tor >/dev/null 2>&1 || true
EOF
  chmod 0755 /usr/local/bin/tl-anon-stop
}
...
if command_exists tor; then
  echo "[PASS] tor: $(command -v tor)"; inc_pass
else
  echo "[WARN] tor: command not found (Tor should be preinstalled in base VM image)"; inc_warn
fi
check_cmd "torsocks" "torsocks"
if command_exists anonsurf; then
  echo "[PASS] AnonSurf: $(command -v anonsurf)"; inc_pass
else
  echo "[WARN] AnonSurf: command not found (best-effort)"; inc_warn
fi
check_cmd "macchanger" "macchanger"
check_cmd "mat2" "mat2"
check_cmd "tl-anon-start wrapper" "tl-anon-start"
check_cmd "tl-anon-stop wrapper" "tl-anon-stop"
